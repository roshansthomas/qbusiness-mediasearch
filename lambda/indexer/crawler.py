...
SUPPORTED_MEDIA_TYPES = ["mp3","mp4","wav","flac","ogg","amr","webm","jpg","png"]

from common import logger
from common import INDEX_ID, DS_ID, STACK_NAME
from common import S3, TRANSCRIBE
from common import start_index_sync_job, stop_index_sync_job_when_all_done, process_deletions, create_newfacets_youtube
...
def process_s3_media_object(crawlername, bucketname, s3url, s3object, s3metadataobject, s3transcribeoptsobject, kendra_sync_job_id, role):
    ...
    if (item == None or item.get("status") == "DELETED"):
        ...
        if suffix.upper() in (mediatype.upper() for mediatype in ["jpg","png"]):
            job_name = "processimage" # Set Job Name to processimage if the extension is an image else create transcription job
        else: 
...
#        make_category_facetable(indexId=INDEX_ID)
    ...
    kendra_sync_job_id = start_index_sync_job(dsId=DS_ID, indexId=INDEX_ID)
    ...
    for bucket in BUCKET_LIST: 
        try:
            ...
            stop_index_sync_job_when_all_done(dsId=DS_ID, indexId=INDEX_ID)
